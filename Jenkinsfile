...
"""
  ...
  stages {
    ...
    stage('Image Vulnerability Scan_By NeuVector') {
      steps {
        neuvector registrySelection: 'harbor_reg', repository: 'library/samples/spring-petclinic', numberOfHighSeverityToFail: "1", numberOfMediumSeverityToFail: "0", tag:"v1.0.${env.BUILD_ID}"
      }
    }
    stage('Approval') {
      input {
